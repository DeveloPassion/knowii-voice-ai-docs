...
From the next release there is one way to add a third, and only by choosing it deliberately: turning on [AI post-processing](./user-guide/ai-post-processing.md) with a provider that is not on your own computer, which asks for your confirmation before it sends anything.
