...
- **Live Dictation**: Text arrives in the target window _while you speak_, chunk by chunk, instead of in one piece when you stop. Does not combine with AI post-processing (the cleanup needs the whole text). See [Live dictation](./user-guide/advanced-settings.md#live-dictation) (coming in the next release)
...
- **The `transcribe` CLI Ships With the App**: The command-line tool is bundled in every installer, no separate download (shipped in 0.8.0) (#31, #135)
- **Cancel Shortcut**: A dedicated hotkey to discard the current recording (shipped in 0.8.0)
...
- **API**: Integrate Knowii Voice AI with other applications and systems programmatically (#143). Scripting and automation are already possible today through the [`transcribe` CLI](./user-guide/cli), which ships inside every installer since 0.8.0 (#47, #131)
