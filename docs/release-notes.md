...
- **Bring your own model file.** Turn on **Use My Own Model Files** in Settings > Transcription, drop a `.gguf` (any family transcribe.cpp knows) or a Whisper `.bin` into the custom models folder, and it appears in the model list with a **Your file** badge — activate it like any other. The app never downloads, checks or deletes these files.
